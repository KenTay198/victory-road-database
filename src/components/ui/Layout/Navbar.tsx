...
const Navbar = ({ className, links, ...props }: IProps) => {
  return (
    <nav {...props} className={["flex flex-col gap-2 w-full py-5 px-2 justify-start", className].join(" ")}>
      {links.map((link) => {
...
const Navlink = ({ url, labelKey, Icon, ...props }: NavLinkProps) => {
  ...
  return (
    <Link href={url.includes(":") ? "#" : url}>
      <div
        className={`flex items-center justify-between px-2 py-1 rounded cursor-pointer duration-200 hover:brightness-75 ${
      ...
      {hasSubLinks && (
        <div className={`border-l ml-4 mt-2 flex flex-col ${expanded ? "block" : "hidden"}`}>
          {subLinks.map((subLink) => {
            return <Navlink key={subLink.url} {...subLink} />;
          })}
        </div>
      )}
    </Link>
